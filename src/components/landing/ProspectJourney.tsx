...
const nflowRoute: Step[] = [
  { icon: "📱", label: "Gets link" },
  { icon: "🎯", label: "Opens nnFlow by Nevorai" },
...
.nnFlow by Nevorai-route-line::after {
  background: #00C896;
...
.nnFlow by Nevorai-end-node { animation: success-pulse 2s ease-out infinite; }

@media (prefers-reduced-motion: reduce) {
  .route-line::after, .youtube-end-node, .nnFlow by Nevorai-end-node {
...
export const ProspectJourney = () => {
  const [activeRoute, setActiveRoute] = useState<"youtube" | "nnFlow by Nevorai">("youtube");

  const renderDesktopRoute = (route: Step[], variant: "youtube" | "nnFlow by Nevorai") => {
        ...
        {route.map((step, i) => {
          const endClass = step.isEnd
            ? isYT
              ...
              : "nnFlow by Nevorai-end-node !w-14 !h-14 !text-emerald-500 font-bold !text-2xl"
            : "";
          ...
          return (
            <Fragment key={i}>
              ...
              {i < route.length - 1 && (
                <div
                  className={`route-line ${isYT ? "youtube-route-line" : "nnFlow by Nevorai-route-line"} flex-1 h-[3px] bg-gradient-to-r ${lineGradient} mx-1 mt-6 min-w-[24px]`}
                >
                  <div
      ...
      <div className="container-app max-w-6xl">
        ...
        >
          ...
          <p className="text-slate-600 max-w-xl mx-auto">
            See exactly where the YouTube route goes wrong — and how nnFlow by Nevorai keeps them on track.
          </p>
        </motion.div>
        ...
        <div className="hidden md:block space-y-14">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <span className="font-bold text-lg text-gradient-brand">✅ nnFlow by Nevorai Route</span>
              <span className="text-sm text-emerald-700/80">— 16–18% conversion</span>
            </div>
            {renderDesktopRoute(nflowRoute, "nnFlow by Nevorai")}
          </motion.div>
        </div>
        ...
        <div className="md:hidden">
          <div className="flex justify-center gap-3 mb-8">
            ...
            <button
              onClick={() => setActiveRoute("nnFlow by Nevorai")}
              className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all min-h-11 ${
                !isYT
                  ...
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center text-xl flex-shrink-0 ${
                      isYT
                        ...
                        : `bg-emerald-500/10 border-2 border-emerald-500/50 ${step.isEnd ? "nnFlow by Nevorai-end-node" : ""}`
                    }`}
                  >
                    {step.icon}
                  </div>
