...
type TabKey = "youtube" | "nnFlow by Nevorai";
...
const nflowSteps: Step[] = [
  { count: 100, label: "Prospects open your nnFlow by Nevorai link", sublabel: "Clean player loads instantly — no distractions anywhere", barWidth: 100 },
...
export const LeakyFunnel = () => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setActiveTab("nnFlow by Nevorai"), 6000);
    return () => clearTimeout(t);
  }, [activeTab]);

  const isDark = activeTab === "nnFlow by Nevorai";
        ...
        <div className="flex justify-center mb-4">
          ...
          >
            ...
            >
              ...
              Without nnFlow by Nevorai
            </button>
            <button
              onClick={() => setActiveTab("nnFlow by Nevorai")}
              className={`flex items-center gap-2.5 px-5 md:px-6 py-3 rounded-xl text-sm md:text-base font-semibold transition-all duration-300 min-h-11 ${
                activeTab === "nnFlow by Nevorai"
                  ? "text-white shadow-lg bg-gradient-brand"
                  : isDark ? "text-gray-400 hover:text-gray-200" : "text-gray-500 hover:text-gray-700"
              }`}
            >
              <span className="text-lg">🎯</span>
              With nnFlow by Nevorai
            </button>
          </div>
        </div>
          ...
          {activeTab === "youtube" ? (
            ...
            >
              ...
              <div className="mt-14 text-center max-w-2xl mx-auto">
                ...
                >
                  See what nnFlow by Nevorai does differently →
                </button>
              </div>
            </motion.div>
          ) : (
            <motion.div
              key="nnFlow by Nevorai"
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -16 }}
              transition={{ duration: 0.4 }}
            >
              <div className="text-center mb-12 max-w-3xl mx-auto">
                <div className="inline-block text-xs font-semibold tracking-wider uppercase text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-full mb-4">
                  The nnFlow by Nevorai Difference
                </div>
                <h2 className="text-3xl md:text-5xl font-bold text-white leading-tight">
                  What Happens When You
                  <br />
                  Send an <span className="text-gradient-brand">nnFlow by Nevorai Link</span> Instead
                </h2>
              ...
              <p className="text-gray-500 text-xs mt-6 text-center max-w-sm mx-auto">
                ...
                nnFlow by Nevorai keeps following up automatically until they're ready.
              </p>
              ...
              <div className="mt-10 max-w-4xl mx-auto">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 p-6 md:p-8 rounded-2xl border border-white/10 bg-white/[0.03]">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 text-xs mt-1">nnFlow by Nevorai conversions</p>
